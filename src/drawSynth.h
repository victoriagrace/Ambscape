...
#ifndef __drawSynth__
#define __drawSynth__
...
#ifndef FAUSTCLASS 
#define FAUSTCLASS drawSynth
#endif

class drawSynth : public dsp {
  private:
	FAUSTFLOAT 	fentry0;
	float 	fRec1[2];
	float 	fConst0;
  ...
  public:
	...
	virtual drawSynth* clone() {
		return new drawSynth();
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addNumEntry("fc_lp", &fentry2, 5e+02f, 12.0f, 3e+03f, 0.01f);
		ui_interface->addNumEntry("freq", &fentry1, 4.4e+02f, 12.0f, 1e+04f, 0.01f);
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		float 	fSlow2 = (0.001f * float(fentry2));
		FAUSTFLOAT* output0 = output[0];
		for (int i=0; i<count; i++) {
			fRec1[0] = (fSlow0 + (0.999f * fRec1[1]));
			fRec4[0] = (fSlow1 + (0.999f * fRec4[1]));
			float fTemp0 = float(max(1e-07f, fabsf(fRec4[0])));
			float fTemp1 = (fRec2[1] + (fConst2 * fTemp0));
			float fTemp2 = (fTemp1 + -1);
			int iTemp3 = int((fTemp2 < 0));
			fRec2[0] = ((iTemp3)?fTemp1:fTemp2);
			float 	fRec3 = ((iTemp3)?fTemp1:(fTemp1 + (fTemp2 * (1 - (fConst1 / fTemp0)))));
			fRec5[0] = (fSlow2 + (0.999f * fRec5[1]));
			float fTemp4 = tanf((fConst3 * fRec5[0]));
			float fTemp5 = (1.0f / fTemp4);
			float fTemp6 = (((fTemp5 + 0.125f) / fTemp4) + 1);
			fRec0[0] = ((fRec1[0] * ((2 * fRec3) + -1)) - (((fRec0[2] * (((fTemp5 + -0.125f) / fTemp4) + 1)) + (2 * (fRec0[1] * (1 - (1.0f / faustpower<2>(fTemp4)))))) / fTemp6));
			output0[i] = (FAUSTFLOAT)((fRec0[0] + (fRec0[2] + (2.0f * fRec0[1]))) / fTemp6);
			// post processing
